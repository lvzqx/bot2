...
class Edit(commands.Cog):
    ...
    class EditModal(ui.Modal):
        ...
        async def _validate_inputs(self) -> Tuple[bool, Optional[str], Optional[str], Optional[bool], Optional[bool], Optional[str]]:
            ...
            Returns:
                Tuple[bool, Optional[str], Optional[str], Optional[bool], Optional[bool], Optional[str]]: 
        ...
        async def on_submit(self, interaction: discord.Interaction) -> None:
            ...
            try:
                # 入力値のバリデーション
                is_valid, error_msg, content, category, is_anonymous, is_private, image_url = await self._validate_inputs()
                ...
                with self._get_db_connection() as conn:
                    ...
                    with conn:
                        # 投稿を更新
                        updated_post = await self._update_post_in_database(
                            conn=conn,
                            ...
                            with self._get_cursor(conn) as cursor:
                                ...
                                if not cursor.fetchone():
                                    await interaction.followup.send(
                                        "❌ 投稿が見つかりません。既に削除されている可能性があります。", 
                                        ephemeral=True
                                    )
                                else:
                                    await interaction.followup.send(
                                        "❌ 編集に失敗しました。自分の投稿のみ編集できます。", 
                                        ephemeral=True
                                    )
                            return
                        
                        # 非公開でない場合はDiscordメッセージを更新
                        if not is_private:
                ...
                logger.error(f"Error in EditModal.on_submit: {error_msg}", exc_info=True)
