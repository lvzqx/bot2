...
class Delete(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            print(f"[DEBUG] 削除リクエスト - ユーザーID: {user_id}, 投稿ID: {post_id}, DM: {isinstance(message.channel, discord.DMChannel)}")
            ...
            with self.bot.db:
                ...
                # DMからのリクエストで公開投稿を削除しようとした場合
                if is_dm and not post[1]:
                ...
                if msg_ref:
                    try:
                        ...
                        print(f"[WARN] メッセージ削除に失敗: {e}")
                
                # 5. DM内の埋め込みメッセージも削除（DMからのリクエストの場合）
                if is_dm:
                    try:
                ...
                # 6. 完了メッセージを送信
                msg_type = "非公開投稿" if post[1] else "投稿"
                await message.channel.send(f"✅ {msg_type} (ID: {post_id}) を削除しました", delete_after=10)
